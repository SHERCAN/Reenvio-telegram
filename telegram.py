...
youtube_regex = re.compile(r"\.?amazon\.com\/\S+")
youtube2_regex = re.compile(r"/amazon\.com/")


class Chrome:
    def optionsChrome(self) -> None:
        self.options = Options()
        argumento = """--user-agent=Mozilla/5.0 (Linux; Android 11; SM-G9910) 
    ...
    def openPage(self, page: str) -> None:
        self.optionsChrome()
        self.driver = uc.Chrome(executable_path=pathChromeDriver,
...
if __name__ == "__main__":
    ...
    async def handle_new_message(event):
        ...
        if prepago_regex.search(text) and comercial_regex.search(text):
            print(f"Mensaje recibido en el grupo {chat_id}: {tarea}")
        if youtube_regex.search(text) or youtube2_regex.search(text):
            print(f"Mensaje con enlace recibido en el grupo {chat_id}:{tarea}")
